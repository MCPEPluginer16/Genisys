...
class Item{
	...
	const IRON_BAR = 101; const IRON_BARS = 101;
	...
	private static function initCreativeItems($readFromJson = false){
		...
		$sections = [
			new Config(Server::getInstance()->getFilePath() . "src/pocketmine/resources/creative_items/building.json", Config::JSON, []),
			new Config(Server::getInstance()->getFilePath() . "src/pocketmine/resources/creative_items/decoration.json", Config::JSON, []),
			new Config(Server::getInstance()->getFilePath() . "src/pocketmine/resources/creative_items/tools.json", Config::JSON, []),
			new Config(Server::getInstance()->getFilePath() . "src/pocketmine/resources/creative_items/miscellaneous.json", Config::JSON, [])
		];
		foreach($sections as $section){
			foreach($section->getAll() as $item){
				if(!isset($item["Disabled"])){
					$newItem = Item::get($item["ID"], $item["Damage"]);
					if(isset($item["Enchantments"])){
						foreach($item["Enchantments"] as $enchantment => $level){
							$newItem->addEnchantment(Enchantment::getEffectByName($enchantment)->setLevel($level));
						}
					}
					self::addCreativeItem($newItem);
				}
			}
		}
	}
